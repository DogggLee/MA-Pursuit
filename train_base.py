...
parser.add_argument('--tensorboard', type=str, help='Checkpoint path')
...
def train_unified(args, config):
    ...
    if config.Model.share:
        print("Training: Use shared policy model")
        hunter_model = MATD3Agent(obs_dim=h_dim,
                            action_dim=config.Hunter.action_dim,
                            lr=config.Train.lr,
                            gamma=config.Train.gamma,
                            tau=config.Train.tau,
        ...
        target_model = MATD3Agent(obs_dim=t_dim,
                            action_dim=config.Target.action_dim,
                            lr=config.Train.lr,
                            gamma=config.Train.gamma,
                            tau=config.Train.tau,
        ...
        for _ in range(env.num_hunter):
            hunter_models.append(hunter_model)
        for _ in range(env.num_target):
            target_models.append(target_model)
        
        hunter_model_inds = [0]
        target_model_inds = [0]
    else:
        hunter_models = [MATD3Agent(obs_dim=h_dim,
                            action_dim=config.Hunter.action_dim,
                            lr=config.Train.lr,
                            gamma=config.Train.gamma,
                            tau=config.Train.tau,
        ...
        target_models = [MATD3Agent(obs_dim=t_dim,
                            action_dim=config.Target.action_dim,
                            lr=config.Train.lr,
                            gamma=config.Train.gamma,
                            tau=config.Train.tau,
                            ...
                            if_lr_decay=config.Model.target_lrdecay,
                            total_episodes=config.Train.num_episodes) for _ in range(env.num_target)]
        hunter_model_inds = list(range(env.num_hunter))
        target_model_inds = list(range(env.num_target))

    if args.checkpoint and Path(args.checkpoint).exists():
        print(f"Loading models from checkpoint: {args.checkpoint}")
        try:
            for i in hunter_model_inds:
                hunter_models[i].load_model(args.checkpoint, agent_id=i, agent_type='hunter')
            for i in target_model_inds:
                target_models[i].load_model(args.checkpoint, agent_id=i, agent_type='target')
            print("Successfully loaded models from checkpoint")
        except Exception as e:
            print(f"Error loading models from checkpoint: {e}")
            print("Training will start with newly initialized models")
    ...
    with open(rewards_csv_path, mode='w', newline='') as csv_file:
        writer = csv.writer(csv_file)
        writer.writerow(["episode", "num_obstacle", "num_hunter", "num_target", 
                         "total_reward_hunters", "total_reward_targets", 
                         "avg_eval_reward_hunters"])
    ...
    for episode in range(config.Train.num_episodes):
        ...
        if config.Train.env_random and config.Train.env_progress_random and episode >= config.Train.env_progress_regen_episode:
            env.re_gen()
        ...
        for step in tqdm(range(config.Train.max_steps)):
            ...
            # hunters choose action
            for i in range(env.num_hunter):
                action = hunter_models[i].select_action(h_obs[i])
                actions_hunters.append(action)

            # targets choose action
            for i in range(env.num_target):
                action = target_models[i].select_action(t_obs[i])
                actions_targets.append(action)
            ...
            if update_counter % config.Train.update_freq == 0:
                if hunters_buffer.size() >= config.Train.min_buffer_size:
                    for _ in range(config.Train.update_iterations):
                        batch = hunters_buffer.sample(config.Train.batch_size)
                        for model_id in hunter_model_inds:
                            items = hunter_models[model_id].update(batch)
                if targets_buffer.size() >= config.Train.min_buffer_size:
                    for _ in range(config.Train.update_iterations):
                        batch = targets_buffer.sample(config.Train.batch_size)
                        for model_id in target_model_inds:
                            items = target_models[model_id].update(batch)
        ...
        if episode % 10 == 0:
            avg_eval_reward = val_unified(args, test_env, 
                                            hunter_models,
                                            target_models,
                                            test_env_cfgs, 
                                            config, exp_dirname,
        ...
        with open(rewards_csv_path, mode='a', newline='') as csv_file:
            writer = csv.writer(csv_file)
            writer.writerow([env.num_obstacle, env.num_hunter, env.num_target,
                             episode, total_reward_hunters, total_reward_targets, 
                             avg_eval_reward])
        ...
        if should_save:
            ...
            for model_id in hunter_model_inds:
                hunter_models[model_id].save_model(save_dir, agent_id=model_id, agent_type='hunter')

            for model_id in target_model_inds:
                target_models[model_id].save_model(save_dir, agent_id=model_id, agent_type='target')

            print(f"Models saved at episode {episode} in {save_dir}")

def val_unified(args, env: MAPursuitEnv, 
              hunter_models: list[MATD3Agent], target_models: list[MATD3Agent], 
              test_env_cfgs, 
              config, 
              val_title,
              gif_dump_root: Path, fps=10):
    ...
    for env_id, test_env_cfg in tqdm(enumerate(test_env_cfgs)):
        ...
        while (not done) and (current_step <= config.Train.max_steps):
            ...
            # hunters choose action
            for i in range(env.num_hunter):
                action = hunter_models[i].select_action(h_obs[i], noise=False)
                actions_hunters.append(action)

            # targets choose action
            for i in range(env.num_target):
                action = target_models[i].select_action(t_obs[i], noise=False)
                actions_targets.append(action)
